...
module.exports = async function handleLol(message, args) {
  ...
  const champArgs = laneMap[args[args.length - 1]?.toLowerCase()]
    ? args.slice(0, -1)
    : args;
  ...
  try {
    ...
    async function dismissCookieModal(targetPage) {
      ...
      for (const frame of frames) {
        try {
          const btn = await frame
            .locator(
              'button.fc-cta-consent, button[aria-label="Consent"], button.fc-primary-button',
            )
            .first();
      ...
      try {
        const btn = await targetPage
          .locator('button[aria-label="Consent"]')
          .first();
        if (await btn.isVisible({ timeout: 500 }).catch(() => false)) {
          await btn.click({ timeout: 5000 });
      ...
      try {
        const btn = await targetPage
          .locator('button:has(p.fc-button-label:has-text("Consent"))')
          .first();
        if (await btn.isVisible({ timeout: 500 }).catch(() => false)) {
          await btn.click({ timeout: 5000 });
    ...
    try {
      await page.evaluate(() => {
        ...
        const selectors = [
          ...
          ".cc-banner",
          ".gdpr-consent",
          '[class*="fc-"][class*="consent"]',
          '[class*="fc-"][class*="modal"]',
          '[class*="cookie"]',
          '[id*="cookie"]',
          '[class*="consent"]',
          '[id*="consent"]',
    ...
    if (buildChoice && buildChoice > 1) {
      try {
        ...
        if (tabs.length >= buildChoice) {
          ...
          console.log(
            `[LOL] Build alternativa #${buildChoice} no encontrada (${tabs.length} tabs), usando default`,
          );
        }
      } catch (e) {
    ...
    await loadingMsg.edit(
      `No pude obtener la build. Puedes verla aqui: ${url}`,
    );
  }
};
