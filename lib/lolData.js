...
async function loadLaneChampions() {
    ...
    try {
        ...
        for (const role of roles) {
            try {
                await page.goto(role.url, { waitUntil: 'domcontentloaded', timeout: 20000 });
                await page.waitForTimeout(2000); // esperar carga inicial
                ...
                for (let scroll = 0; scroll < 30; scroll++) {
                    ...
                    if (currentCount === previousCount) {
                        stableRounds++;
                        if (stableRounds >= 3) break; // 3 rondas sin cambios = fin
                    } else {
                        stableRounds = 0;
                ...
                const names = await page.evaluate(() => {
                    ...
                    // Selectores específicos de la fila de la tierlist
                    const selectors = [
                    ...
                    // Si no hay nada con esos selectores, fallback a hrefs con filtro
                    if (results.size === 0) {
                        const links = document.querySelectorAll('a[href^="/lol/champions/"]');
                        for (const link of links) {
                            const href = link.getAttribute('href') || '';
                            // Solo links que apuntan a build de un campeon especifico (no genericos)
